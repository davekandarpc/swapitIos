...
export const DropDown_dealership = ({
  menuData,
  ...
  return (
    <View style={DropdownPropStyle}>
      ...
      >
        ...
        <Text style={{ fontSize: 14, flex: 1, textAlign: 'center' }}>{value !== null ? value.name : placeholder}</Text>
        {showArrow ?
            ...
            <ScrollView nestedScrollEnabled={true} keyboardShouldPersistTaps="always">
              {
                menuData.length > 0 ? menuData.map((item) => {
                  console.log("menuData 0 ::"+JSON.stringify(item))
                  return (
                    <View
